...
// sc_add
struct prepareNextBatchBeamParam {
    RequestManager *rm;  
...
RequestManager::RequestManager()
    : verbose(false), next_available_guid(1000000), num_processed_requests(0) { // verbose false true
...
int RequestManager::register_ssm_model(FFModel *model) {
  // 每次都取当前 models vector中的长度 来表示当前register的model的id
...
RequestManager::RequestGuid
  ...
  // 将这些prompt encode成的tokens都insert到request.token vector的最后 
  request.tokens.insert(request.tokens.end(), tokens.begin(), tokens.end());
  request.initial_len = request.tokens.size();
  log_req_mgr.print() << "request.initial_len: " << request.initial_len ;
...
BeamSearchBatchConfigFuture RequestManager::prepare_next_batch_beam(
  ...
  auto elapsed1 = std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0);
  // log_req_mgr.print() << "prepare_next_batch_beam (before execute_task) time: " << elapsed1.count() * 1e-6 << " ms."; // 几us 
  return runtime->execute_task(ctx, launcher);
}
...
BeamSearchBatchConfig RequestManager::prepare_next_batch_beam_task(
  ...
  log_req_mgr.print() << "Task ID: " << task->task_id;
  auto t1 = std::chrono::high_resolution_clock::now();
  BeamInferenceResult const &result =
      Future(task->futures[1]).get_result<BeamInferenceResult>(); // 10 ms
  auto t2 = std::chrono::high_resolution_clock::now();

  // 将当前时间转换为持续时间（自纪元以来的微秒数）
  auto timeSinceEpoch = t2.time_since_epoch();
    // 将时间转换为微秒
  ...
  // log_req_mgr.print() << "Future(task->futures[1]).get_result<>() time: " << elapsed1.count() * 1e-6 << " ms.";
  log_req_mgr.print() << "(2 get_result)prepare_next_batch_beam_task: " << elapsed2.count() * 1e-6 << " ms."; // 20us

  return rm->prepare_next_batch_beam(bc, result);  // 十几us
}
...
BeamSearchBatchConfig
  ...
  log_req_mgr.print() << " prepare_next_batch_beam_3 time:" << elapsed2.count() ;
  return new_bc;
}
...
TreeVerifyBatchConfigFuture RequestManager::prepare_next_batch_verify(
  ...
  log_req_mgr.print() << "Enter function TreeVerifyBatchConfigFuture RequestManager::prepare_next_batch_verify";
  Runtime *runtime = Runtime::get_runtime();
  Context ctx = Runtime::get_context();
...
TreeVerifyBatchConfig RequestManager::prepare_next_batch_verify_task(
    ...
    Context ctx,
    Runtime *runtime) {
  log_req_mgr.print() << "Enter function TreeVerifyBatchConfig RequestManager::prepare_next_batch_verify_task";
  RequestManager *rm = *((RequestManager **)task->args);
  // 创建一个old_batches的vector，接收future的计算结果
  ...
  auto t2 = std::chrono::high_resolution_clock::now();
  auto elapsed2 = std::chrono::duration_cast<std::chrono::nanoseconds>(t2-t1);
  // log_req_mgr.print() << "waiting SSMs get result time: " << elapsed2.count() * 1e-6 << " ms."; // 这个时间也是不准确的，没有意义
...
TreeVerifyBatchConfig RequestManager::prepare_next_batch_verify(
  ...
  for (int i = 0; i < TreeVerifyBatchConfig::max_requests_per_batch(); i++) {
    if (old_batches.at(0).request_completed[i]) { // 如果第i个request已经完成了 那么处理下一个request
    // 这里为什么只at(0) 
      continue;
...
GenerationResult FFModel::generate(std::vector<std::string> &prompts,
  ...
  if (rm->get_num_ssms() == 0) { // FFModel.models.size()
...
GenerationResult RequestManager::generate_incr_decoding(
  ...
  while (!is_request_completed(guid)) {
    ...
    auto elapsed1 = std::chrono::duration_cast<std::chrono::nanoseconds>(end1 - begin);

    // 如何测量推理一步产生的token数量？
    // incre_decoding 每一步产生一个token

    log_req_mgr.print() << "LLM incre_decoding inference one token time:" << elapsed1.count()* 1e-9 << " s.";
...
/*static*/
// 与generate_incr_decoding 结构完全一样
GenerationResult RequestManager::generate_spec_infer(
    FFModel *llm, std::vector<std::string> &prompts, int max_seq_length) {
  InferenceManager *im = InferenceManager::get_inference_manager();
  RequestGuid guid;  // size_t  无符号整数类型
  for (int i = 0; i < prompts.size(); i++) {
    // 针对数据集中的每一个prompt 都分别register一次
    guid = register_new_request(prompts.at(i), max_seq_length);
    log_req_mgr.print() << "guid:" << guid;
  }
  if (guid == 0) {
    std::cout
        << "=========== Discard request exceed prompt maximum... ==========="
  ...
  while (!is_request_completed(guid)) {
    ...
    while (batch_pipeline.size() > 1) {
      auto const &batch = batch_pipeline.front();
      if (batch.second.is_ready()) {  // 如果这个Future已经完成 不需要等待 就移出队列
        batch_pipeline.pop();
      } else {
        break;
      }
    ...
    std::vector<BeamSearchBatchConfigFuture> beam_bcf_vec(get_num_ssms()); // 有几个SSM，就将vector长度初始化为多少
    ...
    for (size_t i = 0; i < get_num_ssms(); i++) {
      ...
      for (int depth = 0; depth < BeamSearchBatchConfig::MAX_BEAM_DEPTH;
        ...
        // log_req_mgr.print() << "SSM " << i << "'s depth " << depth << " inference finished!";
        // 程序逻辑要求每个 FutureMap 对象中只有一个 Future 对象 也就是每个推理步骤推理一个token
        // fm.wait_all_results();
        
        assert(fm.get_future_map_domain().get_volume() == 1); // 确保FutureMap中只有一个Future对象

        // 这是一个非阻塞的调用，也就是说调用get_future不阻塞，直接返回一个对象
        BeamInferenceResultFuture beam_irf = fm.get_future(0);

        // beam_irf.get_result<BeamInferenceResult>();
        auto tt2 = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(tt2 - tt1);
        log_req_mgr.print() << "SSMs incre_decoding inference one token time:" << (elapsed).count()* 1e-6 << " ms.";
      ...
      // log_req_mgr.print() << "SSM " << i << " inference finished!" ;
    }
    auto end1 = std::chrono::high_resolution_clock::now();
    auto elapsed1 = std::chrono::duration_cast<std::chrono::nanoseconds>(end1 - begin);
    log_req_mgr.print() << "SSMs inference time:" << elapsed1.count()* 1e-9 << " s."; // 这里的时间应该是完成 启动 所有的SSMs任务的时间
    // Token Tree Verification
    // log_req_mgr.print() << "Start Token Tree Verification " ;
    {
      // 准备下一个iteration的batch数据
      // 将SSMs推理的结果送入 LLM进行verify
      TreeVerifyBatchConfigFuture tree_bcf =
          prepare_next_batch_verify(beam_bcf_vec);
      // 进行推理
      log_req_mgr.print() << "Next Line is LLM FutureMap fm = im->inference";
    ...
    auto elapsed2 = std::chrono::duration_cast<std::chrono::nanoseconds>(end2 - end1);
    log_req_mgr.print() << "LLM Tree Verification time:" << elapsed2.count()* 1e-9 << " s.";
  ...
  GenerationResult gr = get_generation_result(guid);
  // assert(gr.output_tokens.size() >= max_seq_length);
  log_req_mgr.print() << "GenerationResult input_text: " << gr.input_text ;
  log_req_mgr.print() << "GenerationResult output_text: " << gr.output_text ;
  log_req_mgr.print() << "GenerationResult total output tokens num: " << gr.output_tokens.size()-gr.input_tokens.size() ;
  return gr;
}
