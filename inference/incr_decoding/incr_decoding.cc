...
void FlexFlow::top_level_task(Task const *task,
  ...
  int total_num_requests = 0;
  {
    ...
    auto total_inference_time = elapsed1.count() * 1e-6; // ms
    log_app.print() << "model generate time: " << total_inference_time << " ms." ;
    auto total_inference_token_nums = result.output_tokens.size()-result.input_tokens.size();
    log_app.print() << "GenerationResult total output tokens num: " <<  total_inference_token_nums;
    log_app.print() << "every token inference time: " << total_inference_time / total_inference_token_nums << " ms/token.";
    log_app.print() << "GenerationResult input_text: " << result.input_text ;
    log_app.print() << "GenerationResult output_text: " << result.output_text ;
