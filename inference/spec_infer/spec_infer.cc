...
LegionRuntime::Logger::Category log_app("spec_infer");
...
void parse_input_args(char **argv,
  ...
  log_app.print() << "paths.cache_folder_path is:" << paths.cache_folder_path ;
  wordfree(&p);
}
...
void get_model_meta(FilePaths &file_paths,
  ...
  model_metadata.llm_model_config_path =
      join_path({file_paths.cache_folder_path,
                 ...
                 "config.json"});
  log_app.print() << "model_metadata.llm_model_config_path is:" << model_metadata.llm_model_config_path ;
  model_metadata.llm_tokenizer_path =
      join_path({file_paths.cache_folder_path,
                 "tokenizers",
                 model_metadata.model_names.llm_model_name});
  log_app.print() << "model_metadata.llm_tokenizer_path is:" << model_metadata.llm_tokenizer_path ;
  model_metadata.llm_weights_path =
      join_path({file_paths.cache_folder_path,
                 "weights",
                 model_metadata.model_names.llm_model_name,
                 use_full_precision ? "full-precision" : "half-precision"});
  log_app.print() << "model_metadata.llm_weights_path is:" << model_metadata.llm_weights_path ;
  ...
  log_app.print() << "model_metadata.llm_model_type is:" << model_metadata.llm_model_type ;
...
void FlexFlow::top_level_task(Task const *task,
                              ...
                              Context ctx,
                              Runtime *runtime) {
  log_app.print() << "Start execute function: top_level_task!";
  ...
  if (model_metadata.llm_model_type == ModelType::LLAMA) {
    LLAMA::create_llama_model(tree_model,
                              ...
                              generationConfig,
                              use_full_precision);
    log_app.print() << "LLM create_llama_model finished!";
    ...
    OPT::create_opt_model(tree_model,
                          ...
                          TREE_VERIFY_MODE,
                          use_full_precision);
    log_app.print() << "LLM create_opt_model finished!";
    ...
    FALCON::create_falcon_model(tree_model,
                                ...
                                TREE_VERIFY_MODE,
                                use_full_precision);
    log_app.print() << "LLM create_falcon_model finished!";
    ...
    MPT::create_mpt_model(tree_model,
                          ...
                          generationConfig,
                          use_full_precision);
    log_app.print() << "LLM create_mpt_model finished!";
  ...
  for (int ssm_id = 0; ssm_id < num_ssms; ssm_id++) {
    ...
    if (model_metadata.ssm_model_types[ssm_id] == ModelType::LLAMA) {
      LLAMA::create_llama_model(beam_model,
                                ...
                                generationConfig,
                                use_full_precision);
      log_app.print() << "SSM create_llama_model finished!";
      ...
      OPT::create_opt_model(beam_model,
                            ...
                            BEAM_SEARCH_MODE,
                            use_full_precision);
      log_app.print() << "SSM create_opt_model finished!";
      ...
      FALCON::create_falcon_model(
          ...
          BEAM_SEARCH_MODE,
          use_full_precision);
      log_app.print() << "SSM create_falcon_model finished!";
      ...
      MPT::create_mpt_model(beam_model,
                            ...
                            generationConfig,
                            use_full_precision);
      log_app.print() << "SSM create_mpt_model finished!";
  ...
  int total_num_requests = 0;
  {
    ...
    log_app.print() << "total_num_requests: " << total_num_requests ;
    

    log_app.print() << "start SPS tree_model.generate" ;
    tree_model.generate(prompts, 128 /*max_sequence_length*/);  // generate token num = 128 - (12+1</s>) = 115
    log_app.print() << "finish SPS stree_model.generate" ;
  }
